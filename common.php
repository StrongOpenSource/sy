...
if(file_exists(dirname(__FILE__)."/@config_private.txt")){
    ...
    file_put_contents("$base_dir/tmp/php_runtime_error_log.php", "<?php exit();\n");
    set_error_handler(function($errno, $msg, $file, $line) {
        global $base_dir;
        if (!(error_reporting() & $errno)) return false;
        $log_message = "// $file: $line - [$errno] $msg\n";
        file_put_contents("$base_dir/tmp/php_runtime_error_log.php", $log_message, FILE_APPEND);
    });
}
...
// 默认模板目录
if(empty($cms_config_tpl_dir)){
    $cms_config_tpl_dir = 'a0064';
}
...
if(
    ...
    if($script_name == '/index.php'){
        $cache_valid_time = 60;
        $cache_file_path = $base_dir . "/html/index.html.gz";
        exec_cache_start($cache_file_path, $cache_valid_time);
    }
}
