...
<body>
    ...
    <div class="content">
        <div class="container">
            <div style="line-height:1.8;text-align:center;padding:20px;font-size:18px;margin:20px 0;word-break:break-all;background:rgba(17,19,38,.8);border:1px solid var(--border);border-radius:var(--radius-lg);">
                <a href="{@class_link}" style="color:var(--primary);text-decoration:none;font-weight:600;margin-right:8px;">[{@class_name}]</a><b style="color:var(--text-main);">{@name}</b>
            </div>

            <div style="font-family:var(--font);font-size:15px;line-height:2;padding:24px;background:rgba(17,19,38,.8);border:1px solid var(--border);border-radius:var(--radius-lg);margin:20px 0;color:var(--text-muted);">
                <span style="color:var(--text-dim);">{base64}【影片格式】：{/base64}</span>mp4<br>
                <span style="color:var(--text-dim);">{base64}【影片大小】：{/base64}</span>{@torrent_size}<br>
                <span style="color:var(--text-dim);">{base64}【影片时长】：{/base64}</span>{@torrent_duration}分钟<br>
                <span style="color:var(--text-dim);">{base64}【分辨率】：{/base64}</span>{@torrent_resolution}<br>
                <span style="color:var(--text-dim);">{base64}【影片预览】：{/base64}</span><br>
...
{@torrent_capture_v2}
            ...
            <div class="hide_mobile" style="text-align:center;padding:16px;">
                <a href='{@var:api_config_bt_client_pc_download_url}' target='_blank' style="color:var(--primary);font-weight:600;">
                    {base64}{@var:api_config_bt_client_pc_download_text}{/base64}
                </a>
            </div>

            <div class="hide_pc" style="text-align:center;padding:16px;">
                <a href='{@var:api_config_bt_client_mobile_download_url}' style="color:var(--primary);font-weight:600;">
